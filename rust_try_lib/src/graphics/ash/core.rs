...
pub struct GraphicsCoreAsh {
    ...
    device: LazyManual<ash::Device>,

    graphics_queue: vk::Queue,
    present_queue: vk::Queue,

    swapchain_loader: LazyManual<ash::extensions::khr::Swapchain>,
...
impl GraphicsCoreAsh {
    pub fn new() -> Self {
        Self {
            ...
            device: LazyManual::new(),

            graphics_queue: vk::Queue::null(),
            present_queue: vk::Queue::null(),

            swapchain_loader: LazyManual::new(),
...
impl GraphicsCoreAsh {
    ...
    fn create_logical_device(&mut self) {
        ...
        if Self::ENABLE_LAYERS {
            create_info = create_info.enabled_layer_names(&Self::LAYER_NAMES);
        }
    ...
    fn create_swapchain(&mut self) {
        ...
        let mut create_info = vk::SwapchainCreateInfoKHR::builder()
            .surface(self.surface_loader.surface())
    ...
    fn update_uniform_buffer(&self, current_image: usize) {
        let time = globals::TIME.time() as f32;
    ...
    pub(crate) fn draw_frame(&mut self) {
        let framebuffer_size = globals::APPLICATION.window().inner_size();
    ...
    fn choose_swap_extent(&self, capabilities: &vk::SurfaceCapabilitiesKHR) -> vk::Extent2D {
        if capabilities.current_extent.width != u32::MAX {
            ...
            let physical_size = globals::APPLICATION.window().inner_size();
    ...
    fn query_swapchain_support(&self, device: vk::PhysicalDevice) -> SwapChainSupportDetails {
        SwapChainSupportDetails {
            capabilities: unsafe {
                self.surface_loader
                    .get_physical_device_surface_capabilities(device, self.surface_loader.surface())
                    .unwrap()
            },
            formats: unsafe {
                self.surface_loader
                    .get_physical_device_surface_formats(device, self.surface_loader.surface())
                    .unwrap()
            },
...
impl GraphicsCore for GraphicsCoreAsh {}
...
impl Drop for GraphicsCoreAsh {
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            if Self::ENABLE_LAYERS {
                self.debug_utils_loader.destroy_messenger();
            }
...
unsafe impl Sync for GraphicsCoreAsh {}
