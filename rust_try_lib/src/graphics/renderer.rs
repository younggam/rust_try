...
impl Renderer {
    ...
    fn create_surface(&mut self, entry: &ash::Entry, window: &winit::window::Window) {
        unsafe {
            self.surface = ash_window::create_surface(entry, &self.instance, window, None)
    ...
    pub fn startup(&mut self, entry: &ash::Entry, window: &winit::window::Window) {
        if self.initialized {
            return;
        }

        self.create_surface(entry, window);
